      ...
      <div className="flex items-end gap-[6px] h-24">
        {data.map((v, i) => (
          <div key={i} className="flex-1 flex flex-col justify-end items-center gap-2 h-full">
            <div className="chart-bar w-full rounded-md" style={{ height: `${v}%`, background: `linear-gradient(to top, #FFFFFF, rgba(16, 185, 129,${0.2 + v/200}))` }} />
            <span className="chart-label text-[8px] font-bold text-white/40 uppercase">{labels[i]}</span>
          </div>
        ))}
      </div>
    </div>
...
/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
   STORY SCENES ΓÇö Content for the pinned section
  ...
  return (
    <section className="h-screen w-full flex flex-col md:flex-row items-center relative overflow-hidden bg-[#000000] px-8 md:px-20 pt-24 pb-8">
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-40">
        <span className="text-[8px] uppercase tracking-[0.4em] font-bold text-white/80">Scroll</span>
...
export default function Landing() {
  ...
  /* ΓöÇΓöÇ Cursor Follower ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  useEffect(() => {
  ...
  /* ΓöÇΓöÇ Scroll Progress ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ 3D Tilt ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  useEffect(() => {
  ...
  useGSAP(() => {
    ...
    /* ΓöÇΓöÇ ACT 1: Pinned Story ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
    const holdTime = 2.5;
    const transTime = 1.2;
    ...
    const storyTl = gsap.timeline({
      scrollTrigger: {
        ...
        start: 'top top',
        end: `+=${sceneCount * 600}`, // Slightly shorter scroll for better feel
        pin: true,
        scrub: 0.1, 
        anticipatePin: 1,
        onUpdate: (self) => {
    ...
    /* ΓöÇΓöÇ ACT 2: Normal Scroll Features ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  /* ΓöÇΓöÇ Magnetic ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  useEffect(() => {
    const move = (e) => {
    ...
    <div ref={container} className="overflow-x-hidden bg-[#000000] text-white" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      */}
      ...
      <div ref={stageRef} className="story-stage">
        {/* Ambient background orbs ΓÇö providing color for the glass to capture */}
        ...
        {/* Story Scenes ΓÇö absolutely positioned, cross-fade via GSAP */}
        {/* Story Scenes ΓÇö absolutely positioned, cross-fade via GSAP */}
        ...
        {/* Progress dots ΓÇö horizontal at bottom */}
        ...
        <div className="scroll-hint">
          <span className="text-[10px] uppercase tracking-[0.5em] font-black text-white/80">Scroll</span>
          <div className="scroll-hint-line bg-white/40" />
        </div>
      </div>


      {/* ΓöÇΓöÇ THE MATRIX MATRIX (BENTO) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
            ...
            <div className="flex flex-col md:flex-row items-center gap-12 md:gap-16 w-full py-12 border-b border-white/[0.03]">
              ...
              <div className="flex-1 w-full max-w-md cin-right relative">
                ...
                <div className="relative rounded-[2rem] bg-[#0A0A0A]/80 backdrop-blur-xl border border-white/10 p-10 flex flex-col items-center justify-center shadow-[0_20px_50px_rgba(0,0,0,0.5),0_0_80px_rgba(255,255,255,0.02)] overflow-hidden">
                  ...
                  <h4 className="text-[11px] font-bold uppercase tracking-[0.2em] text-white/60 mb-8 w-full text-center">Live Diagnostics</h4>
              ...
              <div className="flex-1 w-full max-w-md cin-left relative">
                ...
                <div className="relative rounded-[2rem] bg-[#0A0A0A]/80 backdrop-blur-xl border border-white/10 p-8 shadow-[0_20px_50px_rgba(0,0,0,0.5),0_0_80px_rgba(255,255,255,0.02)] overflow-hidden">
                  ...
                  <div className="space-y-4">
                    {[
                      { icon: '📺', name: 'Netflix Premium', cost: '₹649/mo', desc: 'Not used in 30 days' },
                      { icon: '🎨', name: 'Canva Pro', cost: '₹499/mo', desc: 'Duplicate subscription' },
                      { icon: '🎵', name: 'Spotify Music', cost: '₹179/mo', desc: 'Price increased by ₹20' },
                    ].map((item, i) => (
                ...
                <div className="relative rounded-[2rem] bg-[#0A0A0A]/80 backdrop-blur-xl border border-white/10 p-8 shadow-[0_20px_50px_rgba(0,0,0,0.5),0_0_80px_rgba(255,255,255,0.02)] overflow-hidden min-h-[300px] flex flex-col">
                  ...
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      ...
                      <p className="text-[10px] text-white/40 mt-1">Goal: ₹40,00,000 in 24 months</p>
                    </div>
                  ...
                  <div className="mt-6 flex justify-between items-center border-t border-white/5 pt-4">
                    <div className="flex flex-col">
                      ...
                      <span className="text-sm font-bold text-white/80">₹12,45,000</span>
                    </div>
                    <div className="flex flex-col text-right">
                      <span className="text-[9px] uppercase font-bold text-white/30">Prediction</span>
                      <span className="text-sm font-bold text-[#10B981]">₹42,10,000</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* ΓöÇΓöÇ Final Call to Action ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
