  ...
  useEffect(() => {
    const unsubscribe = onAuthChange(async (firebaseUser) => {
      ...
      try {
        ...
        // Behavioral & Financial Data Extraction (localStorage + Firestore)
        const savedHealth = localStorage.getItem('finsight_health_score');
        const savedWaste = localStorage.getItem('finsight_subscriptions');
        const savedGoal = localStorage.getItem('finsight_goal_active');
        const savedIncome = localStorage.getItem('finsight_health_monthly_income') || 
                           localStorage.getItem('finsight_income');
        
        const subscriptions = savedWaste ? JSON.parse(savedWaste) : [];
        ...
        if (savedHealth || savedWaste || savedIncome) {
          const healthScore = savedHealth ? parseFloat(savedHealth) : 0;
          const income = savedIncome ? parseFloat(savedIncome) : 0;
          
          setMetrics({
            healthScore: healthScore > 0 ? healthScore.toFixed(1) : "0",
            healthBand: healthScore > 75 ? 'Sovereign' : (healthScore > 50 ? 'Tactical' : (healthScore > 0 ? 'Exposed' : 'Awaiting Audit')),
            velocity: income > 0 ? ((income - wasteTotal) / income).toFixed(2) : "0.00",
            leakage: wasteTotal,
            wasteBand: wasteTotal < 1000 ? 'Optimized' : (wasteTotal < 5000 ? 'Moderate' : 'High Leakage'),
            goalActive: !!savedGoal,
            status: 'Synchronized',
            subs: wasteSubs.slice(0, 2)
          });
        }
      } catch (err) {
      ...
      <main className="flex-1 ml-24 p-8 lg:p-12 space-y-12 relative z-10">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-1">
            ...
            <p className="text-[10px] font-medium text-white/20 uppercase tracking-[0.2em]">Authorized: {user?.displayName || user?.email} / Node: Alpha-Z</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard 
            label="Health Score" 
            value={parseFloat(metrics.healthScore) > 0 ? metrics.healthScore : "N/A"} 
            trend={parseFloat(metrics.healthScore) > 0 ? "+1.2%" : null}
            progress={0}
            sub={parseFloat(metrics.healthScore) > 0 ? "Trajectory: Alpha" : "Audit Required"} 
          />
          <StatCard 
            label="Wealth Velocity" 
            value={metrics.velocity !== "0.00" ? `${metrics.velocity}v` : "0.00v"} 
            progress={0}
            sub={metrics.velocity !== "0.00" ? "Efficiency: Optimized" : "Awaiting Data"} 
          />
          <StatCard 
            label="Budget Leakage" 
            value={`₹${formatINR(metrics.leakage)}`} 
            color={metrics.leakage > 2000 ? "red" : "primary"}
            progress={0}
            sub={metrics.leakage > 0 ? "Identified Nodes" : "Protocol: Secured"} 
          />
          <div className="p-6 flex flex-col justify-between border border-white/5 bg-[#0A0A0A] rounded-[1.5rem] group hover:border-primary/20 transition-all">
            ...
            <div>
                <h3 className="text-xl font-black italic text-white tracking-widest uppercase">{metrics.status}</h3>
                ...
                <div className="p-8 h-full space-y-8 bg-[#0A0A0A] border border-white/5 rounded-[2.5rem] relative overflow-hidden backdrop-blur-xl hover:border-primary/30 transition-all duration-500">
                        ...
                        <div className="relative flex items-center justify-center">
                            <svg className="w-28 h-28 transform -rotate-90 overflow-visible" viewBox="0 0 96 96">
                                ...
                                <Motion.circle 
                                    ...
                                    animate={{ strokeDashoffset: 264 - (264 * (metrics.healthScore || 0)) / 100 }}
                                    transition={{ duration: 2, ease: "easeOut", delay: 0.2 }}
                                    className="text-primary drop-shadow-[0_0_15px_rgba(16,185,129,0.3)]"
                                    strokeLinecap="round"
                                />
                            </svg>
                            <span className="absolute inset-0 flex items-center justify-center text-3xl font-black italic">{metrics.healthScore > 0 ? metrics.healthScore : 0}</span>
                        </div>
                        <div className="space-y-4">
                            <div>
                                <p className="text-[10px] uppercase font-bold text-white/30">Current Band</p>
                                <p className="text-2xl font-bold text-primary italic">{metrics.healthBand || "Awaiting Audit"}</p>
                            </div>
                    ...
                    <div className="pt-6 border-t border-white/5 grid grid-cols-2 gap-4 relative z-10">
                        <div className="bg-white/5 p-4 rounded-2xl">
                            ...
                            <p className="text-xs font-medium text-white/80">{metrics.healthScore > 0 ? "Highly disciplined habits detected across sectors." : "Run audit to initialize behavioral mapping."}</p>
                        </div>
                        <div className="bg-white/5 p-4 rounded-2xl">
                            <p className="text-[8px] uppercase font-bold text-white/30 mb-1">Trajectory</p>
                            <p className="text-xs font-medium text-white/80">{metrics.healthScore > 0 ? "Projected: +4.2 points / 30d" : "Pending health intelligence scan."}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="p-8 h-full space-y-8 bg-[#0A0A0A] border border-white/5 rounded-[2.5rem] relative overflow-hidden backdrop-blur-xl hover:border-red-500/20 transition-all duration-500">
                    ...
                    <div className="space-y-5 relative z-10">
                        <div className="flex items-end gap-3 font-display">
                            <span className="text-4xl font-bold text-red-500">₹{formatINR(metrics.leakage)}</span>
                            <span className="text-xs text-white/30 mb-1.5 uppercase font-bold">Monthly Leakage</span>
                        </div>
                        
                        <div className="space-y-2">
                            {metrics.subs && metrics.subs.length > 0 ? metrics.subs.map((s, i) => (
                                <div key={i} className="flex justify-between items-center bg-white/[0.03] p-3 rounded-xl border border-white/5 group-hover:border-red-500/10 transition-all">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <span className="text-[9px] uppercase font-black text-red-500/60 tracking-tighter">Live Node</span>
                                ...
                                <p className="text-[10px] uppercase font-black text-white/10 tracking-[0.3em] py-4">No leakage nodes identified.</p>
                            )}
                        </div>
                    </div>
                ...
                <div className="p-8 h-full space-y-12 bg-[#0A0A0A] border border-white/5 rounded-[2.5rem] relative overflow-hidden backdrop-blur-xl hover:border-primary/20 transition-all duration-500">
                    ...
                    {metrics.goalActive ? (
                        ...
                        <div className="py-12 flex flex-col items-center justify-center border border-dashed border-white/5 rounded-[2rem] space-y-4">
                            ...
                            <Link to="/goals" className="px-6 py-2 bg-primary/10 border border-primary/20 text-primary text-[8px] font-black uppercase tracking-widest rounded-full hover:bg-primary hover:text-black transition-all">Initialize Architect</Link>
                        </div>
                    )}
                </div>
            </div>
                ...
                <div className="p-8 h-full space-y-8 bg-[#0A0A0A] border border-primary/20 rounded-[2.5rem] relative overflow-hidden backdrop-blur-xl">
                    ...
                    <div className="space-y-4 relative z-10 flex-1">
                        ...
                        <div className="bg-white/5 p-4 rounded-2xl space-y-2 border border-white/5 hover:border-primary/20 hover:bg-white/10 transition-all cursor-pointer group">
                            ...
                            <p className="text-xs text-white/70 leading-relaxed font-medium">Shift 5% of monthly waste to Goal Alpha to save 3 months.</p>
                        </div>
                    </div>
                </div>
            </div>
